...
def compute_motion_covariance(relative_pose: np.ndarray,
    ...
    delta_trans = np.sqrt(dx**2 + dy**2)       # how far we translated
    delta_rot = np.abs(dtheta)               # how much we rotated
    #sigma x defined by how much we've translated, and how much we've rotated (each one's effect defined by a1 and a2)
    #sigma y defined by how much we've translated, and how much we've rotated (each one's effect defined by a1 and a2)
    #sigma theta defined by how much we've translated, and how much we've rotated (each one's effect defined by a3 and a4)

...
class SlamNode(Node):
    ...
    def _should_add_keyframe(self, current_pose: np.ndarray,
        ...
        dtheta = np.abs(utils.normalize_angle(current_pose[2] - last_keyframe_pose[2]))
    ...
    def _process_keyframe(self, scan_msg: LaserScan):
        ...
        # ==============================================================
        node_id = self.pose_graph.add_node(self.current_odom_pose)  # add node to the pose graph (add current odom pose)
        ...
        if self.last_keyframe_pose is not None and self.last_keyframe_scan is not None:
            ...
            #   covariance = odom_cov
            # ==============================================================
            self.pose_graph.add_edge(node_id - 1, node_id, odom_relative, odom_cov) #add an edge beween nodes
        ...
        if self.keyframe_count> 1 and self.keyframe_count % self.optimization_interval == 0:
            ...
            self.occupancy_grid.update(self.current_odom_pose, ranges, scan_msg.angle_min, angle_increment)

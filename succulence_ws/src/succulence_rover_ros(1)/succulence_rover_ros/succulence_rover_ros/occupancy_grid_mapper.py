...
class OccupancyGrid:
    ...
    def world_to_grid(self, x: float, y: float) -> Tuple[int, int]:
        ...
        col = int( (x - self.origin_x )/self.resolution) #converts world frame to grid (columns)
        row = int( (y - self.origin_y )/self.resolution) #converts world frame to grid (row)
        return row, col
    ...
    def update(self, pose: np.ndarray, ranges: np.ndarray,
        ...
        # Compute lidar position in local frame (apply mounting offset)
        ...
        for i, r in enumerate(ranges):
            ...
            end_x = lidar_x + r * np.cos(beam_angle)  # calculates x position of lidar reading (in the world frame) using simple geometry
            end_y = lidar_y + r * np.sin(beam_angle)   # calculates y position of lidar reading (in the world frame) using simple geometry
